...
impl Graph{
    ...
    pub fn add_direction(&mut self, edges: &ListOfEdges) {
        for (u, v) in edges {
    ...
    pub fn create_directed(n: usize, edges: &ListOfEdges) -> Graph {
        ...
        g.add_direction(edges);
        g.sort_graph_lists();
        g
    }
    //creates an adjacency list w/ the undirected graph and reversed edges
    pub fn create_adjacent(n:usize, edges:&ListOfEdges) -> Graph {
        let mut g = Self::create_directed(n,edges);
        g.add_direction(&reverse_edges(edges));
        g.sort_graph_lists();
        g       
    }
}
...
//compute the degree of each node (essentially # of friends)
